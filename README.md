...
以下是我对 AI 生成代码中 2 处关键逻辑的理解（用我自己的话解释）：

1. 移动与合并逻辑：游戏的核心数据其实是一个 4x4 的二维数组。当按下方向键时，代码并不是直接移动方块，而是遍历数组，先去掉空格，再把相邻且数值相同的数字加起来，最后在剩余的空格中随机生成一个 2 或 4。
2. AI是如何自动寻路的：AI其实没有采用“作弊”手段去猜下一个数字在哪里，而是使用了“期望极大极小算法”。通俗地说，它像一个棋手，会提前想象自己走完这一步之后，系统可能在剩下的空位里随机生成 2 或 4，然后它再推演自己接下来的几步怎么走。通过计算所有可能性的得分期望，它选出“当前最稳健、未来最不容易死路”的那一步。
